...
public class BaseViewController extends Controller implements Initializable {
    ...
    public static Predicate<MarcaDto> obtenerUltimaMarca(LocalDate fecha) {
        return (MarcaDto m) -> {
            return m.getMarcajornada().equals(fecha);
        };
    }
...
//                Optional<MarcaDto> ultima = null;
                ...
                if (marcasEmpleado.isEmpty()) {
                    ...
                    marcaDto = marcasEmpleado.stream()
                            .filter(obtenerUltimaMarca(LocalDate.now())) //me filta por jornada
                            .filter(x -> x.getMarcahoraEntrada() == null
                            || // si existen más jornadas, me filta por una jornada incompleta
                            x.getMarcahoraSalida() == null)
                            .findFirst()
                            .orElse(null);

                    if (marcaDto == null) {
                        marcaDto = new MarcaDto();
                        marcaDto.crearMarca(LocalDateTime.now(), true);                      //si el empleado no tiene marcas, le crea una
                    } else {  // sino actualiza la que encontró en los filtros
                        //Hay dos formas de actualizar, si excede las 12 horas o no, si las excede crea una nueva marca, sino actualiza la filtrada
                        Long diferenciaHorasEntrada = null;
                        Long diferenciaHorasSalida = null;
                        boolean bandera = false;
                        if (marcaDto.getMarcahoraEntrada() != null) {
                            diferenciaHorasEntrada = HOURS.between(marcaDto.getMarcahoraEntrada(), LocalDateTime.now());
